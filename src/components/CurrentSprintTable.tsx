...
export default function CurrentSprintTable({ issues, activeSprint }: { issues: Issue[], activeSprint: any }) {
  ...
  // const getSprintName = (sprintId: string | null) => { // Removed unused function
      ...
      <div className="bg-gradient-to-r from-blue-50 to-indigo-50 border-b border-slate-200 px-6 py-4">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h2 className="text-xl font-bold text-slate-800">{activeSprint.name}</h2>
              <p className="text-sm text-slate-600">
                {new Date(activeSprint.startDate).toLocaleDateString('pl-PL')} → {new Date(activeSprint.endDate).toLocaleDateString('pl-PL')}
              </p>
            </div>
          </div>
          <div className="text-right">
            <div className="text-sm text-slate-600">
              <span className="font-semibold text-blue-600">{filteredIssues.length}</span> / {issues.length} issues
            </div>
            <div className="text-xs text-slate-500">in current sprint</div>
          </div>
        </div>
      </div>
